...
// ─── TV IR code database ─────────────────────────────────────────
// NEC codes sourced from widely-published universal remote databases.
// ⚠️ Different TV models (even same brand) may use different code sets.
// These are the most common codes; probing lets users verify if they work.
...
static const TVBrand TV_BRANDS[] = {
    // ── NEC protocol TVs ──
    // Hisense (海信): common NEC set, addr 0x00
    {"hisense",     NEC,  0x00, 0x12, 0x02, 0x03, 0x01, 0x00, 0x0A, 0x0B, {0x10,0x11,0x12,0x13,0x14,0x15,0x16,0x17,0x18,0x19}},
    // Skyworth (创维): distinct from Hisense
    {"skyworth",    NEC,  0x00, 0x0C, 0x0D, 0x0E, 0x01, 0x02, 0x0B, 0x0F, {0x01,0x02,0x03,0x04,0x05,0x06,0x07,0x08,0x09,0x00}},
    // Changhong (长虹): some models share Hisense codes, this is an alternate common set
    {"changhong",   NEC,  0x00, 0x12, 0x02, 0x03, 0x04, 0x05, 0x0A, 0x0E, {0x10,0x11,0x12,0x13,0x14,0x15,0x16,0x17,0x18,0x19}},
    // Konka (康佳)
    {"konka",       NEC,  0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x14, 0x08, {0x10,0x11,0x12,0x13,0x14,0x15,0x16,0x17,0x18,0x19}},
    // Xiaomi (小米): Mi TV / Mi Box IR remote
    {"xiaomi",      NEC,  0x00, 0x15, 0x16, 0x17, 0x18, 0x19, 0x0D, 0x0F, {0x00,0x01,0x02,0x03,0x04,0x05,0x06,0x07,0x08,0x09}},
    // Sharp (夏普): NEC
    {"sharp",       NEC,  0x00, 0x15, 0x02, 0x03, 0x01, 0x00, 0x0A, 0x1A, {0x10,0x11,0x12,0x13,0x14,0x15,0x16,0x17,0x18,0x19}},

    // ── Sony SIRC (12-bit: 7-bit command + 5-bit device) ──
    {"sony",        SONY, 0x01, 0x15, 0x12, 0x13, 0x10, 0x11, 0x14, 0x25, {0x00,0x01,0x02,0x03,0x04,0x05,0x06,0x07,0x08,0x09}},

    // ── Philips RC5 (5-bit address + 6-bit command) ──
    {"philips",     RC5,  0x00, 0x0C, 0x10, 0x11, 0x20, 0x21, 0x0D, 0x38, {0x00,0x01,0x02,0x03,0x04,0x05,0x06,0x07,0x08,0x09}},
